...
def calculate_demographic_data(print_data=True):
    ...
    # df['sex'] == 'Male' creates a Boolean mask for all rows where sex is Male.
    # df[df['sex'] == 'Male']- selects only row where sex is male
    # ['age'] - selctes age column of these rows
    # .mean() - calculates average age and round to 1 decimal place

    # What is the percentage of people who have a Bachelor's degree?
    percentage_bachelors = round((df['education'].value_counts(normalize=True)['Bachelors'])*100, 1)
    # df['education'].value_counts(normalize=True) counts unique values as a fraction of total rows.
    # ['Bachelors'] selects the fraction of people with Bachelors, then multiply with 100 and rounds to 1 decimal place
